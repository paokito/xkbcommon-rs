...
pub type Keycode = xkb_keycode_t;
...
pub type LayoutIndex = xkb_layout_index_t;
/// A mask of layout indices
pub type LayoutMask = xkb_layout_mask_t;
...
pub type LevelIndex = xkb_level_index_t;
...
pub type ModIndex = xkb_mod_index_t;
/// A mask of modifier indices.
pub type ModMask = xkb_mod_mask_t;
...
pub type LedIndex = xkb_led_index_t;
/// A mask of LED indices.
pub type LedMask = xkb_led_mask_t;
...
pub type KeysymFlags = xkb_keysym_flags;
pub const KEYSYM_NO_FLAGS: KeysymFlags = 0;
pub const KEYSYM_CASE_INSENSITIVE: KeysymFlags = 1 << 0;
...
pub const CONTEXT_NO_DEFAULT_INCLUDES: ContextFlags = 1 << 0;
/// Don't take RMLVO names from the environment.
pub const CONTEXT_NO_ENVIRONMENT_NAMES: ContextFlags = 1 << 1;

#[repr(C)]
pub enum LogLevel {
    Critical = xkb_log_level::CRITICAL as isize,
    Error = xkb_log_level::ERROR as isize,
    Warning = xkb_log_level::WARNING as isize,
    Info = xkb_log_level::INFO as isize,
    Debug = xkb_log_level::DEBUG as isize,
}
...
pub type KeymapFormat = xkb_keymap_format;
...
pub const KEYMAP_FORMAT_USE_ORIGINAL: KeymapFormat = 0xffff_ffff;

/// Specifies the direction of the key (press / release).
#[repr(C)]
pub enum KeyDirection {
    /// the key was released
    Up = xkb_key_direction::UP as isize,
    /// the key was pressed
    Down = xkb_key_direction::DOWN as isize,
}
...
pub type StateComponent = xkb_state_component;
...
/// the other modes.
pub type StateMatch = xkb_state_match;
///Returns true if any of the modifiers are active.
pub const STATE_MATCH_ANY: StateMatch = 1 << 0;
///Returns true if all of the modifiers are active.
pub const STATE_MATCH_ALL: StateMatch = 1 << 1;
/// Makes matching non-exclusive, i.e. will not return false if a
///  modifier not specified in the arguments is active.
pub const STATE_MATCH_NON_EXCLUSIVE: StateMatch = 1 << 16;
...
/// See `xkb_keycode_t`.
#[must_use]
pub fn keycode_is_legal_ext(key: Keycode) -> bool {
    key <= KEYCODE_MAX
}
...
#[must_use]
pub fn keycode_is_legal_x11(key: Keycode) -> bool {
    (8..=255).contains(&key)
}

/// Get the name of a keysym.
#[must_use]
pub fn keysym_get_name(keysym: Keysym) -> Option<String> {
    unsafe {
        ...
        Some(String::from_utf8_unchecked(slice.to_owned()))
    }
}
...
#[must_use]
pub fn keysym_from_name(name: &str, flags: KeysymFlags) -> Option<Keysym> {
    unsafe {
        let cname = CString::new(name.as_bytes().to_owned()).unwrap();
        match xkb_keysym_from_name(cname.as_ptr(), flags) {
            KEY_NoSymbol => None,
            keysym => Some(keysym),
        }
    }
...
pub fn keysym_to_string(keysym: Keysym) -> Option<String> {
    unsafe {
        ...
        let slice: &[u8] = slice::from_raw_parts(ptr as *const _, len as usize);
        String::from_utf8(slice.to_owned()).ok()
    }
}
...
pub fn keysym_to_char(keysym: Keysym) -> Option<char> {
    match unsafe { xkb_keysym_to_utf32(keysym) } {
        0 => None,
        c => char::from_u32(c),
    }
}
...
#[must_use]
pub fn char_to_keysym(ucs: char) -> Option<Keysym> {
    match unsafe { xkb_utf32_to_keysym(ucs as u32) } {
        KEY_NoSymbol => None,
        keysym => Some(keysym),
    }
}
...
impl Keymap {
        ...
        F: FnMut(&Keymap, Keycode),
    {
        ...
        unsafe {
            xkb_keymap_key_for_each(self.get_raw_ptr(), Self::callback::<F>, data_ptr);
